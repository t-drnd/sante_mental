...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('/api/ai/daily-chat', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[#a0a0a0] mt-2">Un espace doux et bienveillant pour partager</p>
      </div>

      <Card className="h-[600px] flex flex-col">
        <CardHeader>
          <CardTitle>Conversation quotidienne</CardTitle>
        </CardHeader>
        <CardContent className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto space-y-4 mb-4">
            {messages.length === 0 && (
              <div className="text-center text-[#a0a0a0] py-8">
                ...
                <p>Comment s'est passée ta journée ?</p>
                <p className="text-sm mt-2">Je suis là pour t'écouter, sans jugement.</p>
              </div>
            )}
          ...
          <div className="flex space-x-2">
            <Input
              ...
              placeholder="Raconte-moi ta journée..."
              disabled={loading}
            />
